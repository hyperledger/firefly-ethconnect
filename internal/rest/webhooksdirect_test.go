...
func newTestWebhooksDirectServer(maxMsgs int) (*webhooksDirect, *httptest.Server, *memoryReceipts, *mockProcessor) {
	...
	wh := newWebhooks(wd, wd.receipts, nil, nil)
	wh.addRoutes(router)
	ts := httptest.NewServer(router)
	return wd, ts, r, p
}

func newTestMsg() messages.SendTransaction {
	return messages.SendTransaction{
...
func TestWebhooksDirectSubmitSendTransaction(t *testing.T) {
	...
	reconstructed := &messages.SendTransaction{}
